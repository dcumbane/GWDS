var FonteRepo = (function() {
  var SHEET = 'Fontes_Dispersas';
  ...
  function list(filtros) {
    ...
    if (filtros.ano) rows = rows.filter(function(r){ return String(r.ano_const) === String(filtros.ano); });
    if (filtros.tipo) rows = rows.filter(function(r){ return String(r.tipo_fonte).toLowerCase() === String(filtros.tipo).toLowerCase(); });
    if (filtros.q) {
  ...
  function create(fonte) { BaseRepo.insert(SHEET, fonte); return fonte.codigo; }
